df_calculation = cli_data[,c("Participant ID", 
                             "Age", "Sex", "Waist_circumference", "Hip_circumference", "WHR", 
                             "Smoking_status", "Previous_smoke", "Current_smoke", "Cystatin_C", 
                             "Cholesterol", "LDL", "HDL", "Triglycerides", "Creatinine", "BMI", 
                             "antiHyp", "statin_usage", "diabetes_status", "hypert_status", 
                             "Creatinine_mg_dl", "eGFRcr_cys", "HbA1c", "DBP", "SBP")]

df = X0818_final_version_scores_MACE
library(data.table)
...
# separate by sex 
cox_to_calib_male =coxph(Surv (time, outcome ) ~linpred_male, data = male)
cox_to_calib_female =coxph(Surv (time, outcome ) ~linpred_female, data = female)

library(nricens)

calibrated_risk_m = get.risk.coxph(cox_to_calib_male, t0 = max(df$time, na.rm = TRUE))
...
df$linear_predictor <- score2(df$Sex, df$Age, df$Current_smoke, df$SBP, df$Cholesterol, df$HDL, 
                              type="linear predictor", calibration=FALSE, weights="excluding UK Biobank")

df$Current_smoke <- ifelse(df$Current_smoke == 1, TRUE, FALSE)

df$absolute_risk <- score2(df$Sex, df$Age, df$Current_smoke, df$SBP, df$Cholesterol, df$HDL, 
                           type="absolute risk", calibration=TRUE, risk_region="low", weights="excluding UK Biobank")

df$absolute_risk_noa <- score2(df$Sex, df$Age, df$Current_smoke, df$SBP, df$Cholesterol, df$HDL, 
                           type="absolute risk", calibration=F)


head(df)
SCORE2 = df[,c("Participant ID","linear_predictor","absolute_risk")]
dput(colnames(SCORE2))
colnames(SCORE2) = c("Participant ID", "SCORE2_linear_predictor", "SCORE2_absolute_risk")
X0818_final_version_scores_MACE = merge(X0818_final_version_scores_MACE,SCORE2,by = "Participant ID")
X0818_final_version_scores_MACE = X0818_final_version_scores_MACE[,-38]
fwrite(X0818_final_version_scores_MACE,file = "1016_finalversion_scores_MACE.csv")
